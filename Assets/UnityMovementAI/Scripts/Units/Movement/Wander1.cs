...
namespace UnityMovementAI
{
    ...
    public class Wander1 : MonoBehaviour
    {
        ...
        public Vector3 GetSteering()
        {
            ...
            Vector3 targetPosition = transform.position + (SteeringBasics.OrientationToVector(characterOrientation, true) * wanderOffset);

            //debugRing.transform.position = targetPosition;

            /* Calculate the target position */
            targetPosition = targetPosition + (SteeringBasics.OrientationToVector(targetOrientation, true) * wanderRadius);
